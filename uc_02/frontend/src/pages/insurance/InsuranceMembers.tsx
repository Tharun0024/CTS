...
import { getPatients as getMembers } from '../../services/patientsApi';
...
export function InsuranceMembers() {
  ...
  const statCounts = {
    ...
    critical: Members.filter(p => p.status === 'Critical').length,
  };
  ...
  return (
    <div className="max-w-7xl mx-auto w-full animate-fade-in-up space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: 'Total Members', value: statCounts.total, icon: Users, gradient: 'from-slate-600 to-slate-800' },
            ...
            <>
                    ...
                    <TableRow className="hover:bg-transparent">
                      <TableCell colSpan={5} className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-slate-500 font-medium text-sm">No Members found matching "{searchTerm}"</p>
                        </div>
                      </TableCell>
                    </TableRow>
                  )}
              ...
              {!loading && filtered.length > 0 && (
                <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 text-xs font-medium text-slate-500">
                  Showing <span className="font-semibold text-slate-700">{filtered.length}</span> of <span className="font-semibold text-slate-700">{Members.length}</span> Members
                </div>
              )}
            </>
          )}
