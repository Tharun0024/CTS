...
export function Denials() {
  const [denials] = useState(mockDenials);
  ...
  const totalDeniedAmount = denials.reduce((acc, d) => acc + d.denied_amount, 0);
  const wonAmount = denials.filter(d => d.status === 'Won').reduce((acc, d) => acc + d.denied_amount, 0);
  const openCount = denials.filter(d => d.status === 'Open').length;
  const inAppealCount = denials.filter(d => d.status === 'In Appeal').length;
  const wonCount = denials.filter(d => d.status === 'Won').length;
  const appealSuccessRate = denials.filter(d => d.status === 'Won' || d.status === 'Lost').length > 0
    ? Math.round((wonCount / (wonCount + denials.filter(d => d.status === 'Lost').length)) * 100)
    : 0;
        ...
        <CardContent className="p-0">
                ...
                <TableRow className="hover:bg-transparent">
                  <TableCell colSpan={5} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-slate-500 font-medium text-sm">No denials found</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
          ...
          {filtered.length > 0 && (
            <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 text-xs font-medium text-slate-500">
              Showing <span className="font-semibold text-slate-700">{filtered.length}</span> of <span className="font-semibold text-slate-700">{denials.length}</span> denials
            </div>
          )}
        </CardContent>
      </Card>
