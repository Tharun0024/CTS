...
export function InsuranceHeader({ setIsMobileOpen, isCollapsed, setIsCollapsed }: InsuranceHeaderProps) {
  ...
  // procedure). A single match opens that claim; otherwise the incoming
  // claims list is opened with the query pre-applied. No IDs are fabricated.
  const handleSearch = async () => {
    ...
    try {
      ...
      if (exact) { navigate(`/insurance/claims/${exact.claim_id}`); return; }
      ...
      if (matches.length === 1) { navigate(`/insurance/claims/${matches[0].claim_id}`); return; }
      navigate(`/insurance/claims?q=${encodeURIComponent(term)}`);
    } catch {
      navigate(`/insurance/claims?q=${encodeURIComponent(term)}`);
    }
  };
