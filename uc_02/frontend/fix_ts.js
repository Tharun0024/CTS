...
// claimsApi.ts
replaceInFile('src/services/claimsApi.ts', /import \{ mockClaims \} from '\.\.\/mock\/claims';\n/, '');
replaceInFile('src/services/claimsApi.ts', /let claimsStore: Claim\[\] = \[\.\.\.mockClaims\];/g, 'let claimsStore: Claim[] = [];'); // Or keep it if we need mock claims... actually mock/claims exports mockClaims, but they are unused if we redefine them? Let's check `claimsApi.ts` later if needed, or just `let claimsStore: Claim[] = (globalThis as any).mockClaims || []`?
// Let's just remove the unused mockClaims import.
